...
class _HomeScreenState extends State<HomeScreen> {
  ...
  getCartLocalData() {
    ...
    if (cartData != null) {
      ...
      // print(userMap);
      cartProducts = List.generate(decodedData.length, (index) =>
          ProductData.fromJson(decodedData[index]));
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<AppCubit , AppStates>(
      ...
      builder: (context , state){
        ...
        return Scaffold(
            drawer: Container(
              width: MediaQuery.of(context).size.width * 0.66, // 66% of screen will be occupied
        color: Colors.grey,
        child: Drawer(
          elevation: 10,
          child: SafeArea(
              ...
              child: Column(
                children: [
                  ...
                  Padding(
                    ...
                    child: InkWell(
                      onTap: (){
                        navigateTo(context, ForumsScreen());
                      },
                      child: Row(
                        children: [
                          Icon(Icons.people,
          ...
          body:  myCubit.homeModel == null ? Center(child: CircularProgressIndicator(),):Padding(
                                      ...
                                      Expanded(
                                        child: GridView.count(
                                          ...
                                          children: List.generate(
                                            myCubit.homeModel!.data.length,
                                                (index) =>
                                                buildGridItem(model: myCubit.homeModel!.data[index], ),
                                          ),
                                        ),
                                      ),
                                    ],
                                    ...
                                    children: [
                                      Expanded(
                                        child: GridView.count(
                                          ...
                                          childAspectRatio: 0.55 ,
                                          children: List.generate(
                                            myCubit.planetModel!.data!.length,
                                                (index) =>
                                                    buildPlanetItem(model: myCubit.planetModel!.data![index], ),
                                          ),
                                        ),
                                      ),
                                    ],
                                    ...
                                    children: [
                                      Expanded(
                                        child: GridView.count(
                                          ...
                                          childAspectRatio: 0.55 ,
                                          children: List.generate(
                                            myCubit.seedModel!.data!.length,
                                                (index) =>
                                                buildSeedItem(model: myCubit.seedModel!.data![index], ),
                                          ),
                                        ),
                                      ),
                                    ],
                                    ...
                                    children: [
                                      Expanded(
                                        child: GridView.count(
                                          ...
                                          childAspectRatio: 0.55 ,
                                          children: List.generate(
                                            myCubit.toolsModel!.data!.length,
                                                (index) =>
                                                buildToolsItem(model: myCubit.toolsModel!.data![index], ),
                                          ),
                                        ),
                                      ),
                                    ],
                ...
                child: MaterialButton(
                  onPressed: (){
                    ...
                    if(cartProducts.isEmpty){
                      ...
                      List<Map<String,dynamic>> productsMap = List.generate(cartProducts.length, (index) => cartProducts[index].toJson());
                      // pass the data to method so its saved.
                      storeCart(productsMap);

                    }else{
                      // لو مليان هبحث عن الاي دي فيهم وازوده
                      if(cartProducts.where((element) => element.productId == model.productId).toList().isNotEmpty) {
                        // fetch the data of the only product have the id of cart item
                        ProductData oldExsitedProduct = cartProducts.where((element) => element.productId == model.productId).toList()[0];
                        // when we find the same product in cart we just add quantity not new product
                        oldExsitedProduct.quantity = model.quantity! + oldExsitedProduct.quantity!;
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          Flexible(
            child: Container(
              ...
              child: model.imageUrl != '' ? Image.network('https://lavie.orangedigitalcenteregypt.com${model. imageUrl!}',
                ...
                height: 100.rSp,
                fit: BoxFit.cover,
              ) : Image.network('https://mir-s3-cdn-cf.behance.net/project_modules/disp/917cf373927307.5c1a39abc8ff6.png',
              width: 100.rSp,
                height: 100.rSp,
              ),
            ),
          ...
          SizedBox(height: 10.rh,),
          Text('Water Capacity : ${model.waterCapacity} %',
            style: TextStyle(
              fontWeight: FontWeight.w600,
              fontFamily: 'Roboto',
              fontSize: 12,
